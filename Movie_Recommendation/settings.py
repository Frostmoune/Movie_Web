...
SECRET_KEY = 'o@*q5!@aj3noz4#8+x)qs%d1m%jg(%3av0720=w97!9=h^x@xi'
...
TEMPLATES = [
    {
        ...
        'DIRS': [os.path.join(SIGN_DIR, 'templates').replace('\\', '/'),],
        'APP_DIRS': True,
        'OPTIONS': {
            'context_processors': [
...
# 这一部分用于把静态文件推到页面上
STATIC_ROOT = os.path.join(SIGN_DIR,'static')
STATIC_URL = '/static/'
STATICFILES_DIRS = (
    ('css',os.path.join(STATIC_ROOT,'css').replace('\\','/')),  
    ('js',os.path.join(STATIC_ROOT,'js').replace('\\','/')), 
    ('image',os.path.join(STATIC_ROOT,'image').replace('\\','/')), 
    ('items',os.path.join(STATIC_ROOT,'items').replace('\\','/')), 
)